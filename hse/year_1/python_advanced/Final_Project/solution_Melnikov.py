...
class SetEncoder(json.JSONEncoder): #Inherits and overrides
...
def build_command(args):
    """Builds an inverted index and saves it according to
    CLI arguments of 'build' command"""
    inverted_index = build_inverted_index(load_document(args.dataset_path))
    inverted_index.dump(args.index_path)


def query_command(args):
    """Processes a query from a specified file according to
        CLI arguments of 'query' command"""
...
def create_argparser():
    ...
    parser = argparse.ArgumentParser(
        description='Creates inverted index and processes query requsts')
